...
public class ConvGen extends Gen {
	...
	public ConvGen() {
		super();
		paramAliases.put("kernelSizes", Arrays.asList("kernelsizes", "kernel_size", "kernel_sizes", "kernel_shape", "kernel", "kernels"));
		paramAliases.put("nodeNumber", Arrays.asList("nodenumber", "node_number","node","nodes","filters"));
		...
		HashMap<String, String> convDefaults = new HashMap<>();
		convDefaults.put("padding", "false");
		defaultValues.put("conv",convDefaults);
	}
...
//				return null;
		...
		if(inputShape == null) {
			...
			for(int i = 0; i <= dimensions; i++) {
				inputShape.add(rand.nextInt(2)+1);//rand.nextInt(2)+1);//rand.nextInt(3)+1);
			}	
		}
		...
		else if(config != null && !config.isEmpty() && config.containsKey("weights") && config.get("weights") != null){
		...
		if(config != null && !config.isEmpty() && config.containsKey("bias") && config.get("bias") != null){
