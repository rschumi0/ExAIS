	...
	public Layer generateLayer(Random rand, String name, List<Integer> inputShape, LinkedHashMap<String, Object> config) {
		...
		if(config != null && !config.isEmpty() && config.containsKey("bias") && config.get("bias") != null){
			if(name.startsWith("GRU")) {
				if(((Object[])config.get("bias")).length == 2) {
					nodeNumber = ((Object[])((Object[])config.get("bias"))[0]).length/3;
				}
				else {
		...
		if(config != null && !config.isEmpty() && config.containsKey("weights") && config.get("weights") != null){
		...
		if(name.startsWith("Simple_RNN")) {
			...
			if(config != null && !config.isEmpty() && config.containsKey("reset_after")){
				reset_after = (Boolean)config.get("reset_after");
			}
			if(reset_after) {
				outWS = new ArrayList<>();
				outWS.add(2);
